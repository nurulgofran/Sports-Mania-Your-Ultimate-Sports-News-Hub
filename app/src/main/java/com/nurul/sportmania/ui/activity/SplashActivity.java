...
public class SplashActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(pref.loadNightModeState()){
            ...
            Picasso.get().load(R.drawable.ic_launcher_dark).placeholder((int) R.drawable.ic_launcher_dark).error((int) R.drawable.ic_launcher_dark).into(logo);
        }else{
            Picasso.get().cancelRequest(logo);
            Picasso.get().load(R.drawable.ic_launcher).placeholder((int) R.drawable.ic_launcher).error((int) R.drawable.ic_launcher).into(logo);
        ...
        call.enqueue(new Callback<Settings>() {
            ...
            public void onFailure(retrofit2.Call<Settings> call, Throwable t) {
                if (t.toString().contains("SocketTimeoutException")) {
                    ...
                    UiUtils.Message(getApplicationContext(), getResources().getString(R.string.timeout_error));
                    pb_settings.setVisibility(View.GONE);
                } else {
                    UiUtils.Write(t.getMessage(), "NewsDetailActivity");
